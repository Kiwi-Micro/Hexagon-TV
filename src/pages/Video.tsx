...
function Video({ videoInfo, db, watchlist }: ProductProps) {
	...
	useEffect(() => {
		const isInWatchlist = watchlist.find((item: any) => item.urlName === videoInfo.urlName);
		setIsInWatchlist(isInWatchlist ? true : false);
	}, []);

	document.title = "Hexagon TV | " + videoInfo.name;
	const ratingInfo = ratings.find((item: any) => item.rating === videoInfo.rating);
	return (
		<div className="main">
			<GlobalNavBar />
			<div className="videoPage heroContainer">
				...
				<div className="homePageHeroInfo">
					...
					{isInWatchlist ? (
						<svg width="32" height="32" viewBox="0 0 16 16" style={{ fill: "#ffffff", marginLeft: "10px", position: "relative", top: "10px" }} xmlns="http://www.w3.org/2000/svg" onClick={() => removeFromWatchlist(videoInfo.urlName, setIsInWatchlist)}>
						...
						<svg width="32" height="32" viewBox="0 0 16 16" style={{ fill: "#ffffff", marginLeft: "10px", position: "relative", top: "10px" }} xmlns="http://www.w3.org/2000/svg" onClick={() => addToWatchlist(videoInfo.name, videoInfo.urlName, videoInfo.thumbnailURL, setIsInWatchlist)}>
